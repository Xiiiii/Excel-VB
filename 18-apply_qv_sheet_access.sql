...
begin
  ...
  dbms_output.put_line('Setting QV Sheet access matrix...');
  ...
  delete from priv p where exists (select 1 from PRIV_qv_sheet pp where pp.priv_id=p.priv_id);
  ...
  FOR i in v_access_list.first.. v_access_list.last LOOP
    ...
    FOR j in v_access.first .. v_access.last-2 LOOP
      ...
      IF  v_access_type = 'Y' THEN
        insert into v_grantee_priv_all (priv_id, grantee_id, distance, grant_access, priority, priv_type, application_id, sheet_id) 
					values ( seq_priv.nextval, v_group_id, 0, v_access_type, 10, 'qv_sheet', v_app_name, v_sheet_name);
        
        dbms_output.put_line('------v_sheet_name:'||v_app_name||':'||v_sheet_name||' v_group_id:'||v_group_id||' access:Y priority:10');
      ELSIF  v_access_type = 'N' THEN
        insert into v_grantee_priv_all (priv_id, grantee_id, distance, grant_access, priority, priv_type, application_id, sheet_id) 
					values ( seq_priv.nextval, v_group_id, 0, v_access_type, 20, 'qv_sheet', v_app_name, v_sheet_name);
        
        dbms_output.put_line('------v_sheet_name:'||v_app_name||':'||v_sheet_name||' v_group_id:'||v_group_id||' access:N priority:20');
      END IF;
    END LOOP;
  END LOOP;
  commit;
